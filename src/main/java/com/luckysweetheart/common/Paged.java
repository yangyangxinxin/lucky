 ...
 *   Paged pg = new Paged();        // 或 new Paged(itemsPerPage);
 *
 ...
 * </p>
 */
public class Paged implements Serializable, Cloneable {
	 ...
	 *            总项数
	 */
	public Paged(int itemsPerPage, int items) {
	 ...
	 *
	 * @param refreshPage 
	 *            是否刷新当前页,调整当前页使之在改变每页项数前后显示相同的项。
	 * 
	 * @return 设置后的每页项数
	 */
	public void setPageSize(int pageSize,boolean refreshPage) {
		int tmp = this.pageSize;

		this.pageSize = (pageSize > 0) ? pageSize : DEFAULT_ITEMS_PER_PAGE;

		if (page > 0 && refreshPage) {
	...
	public int getOffset() {
		return (page > 0) ? (pageSize * (page - 1)) : 0;
	}
	...
	public int getEndIndex() {
		if (page > 0) {
			return Math.min(pageSize * page, items);
		} else {
			return 0;
		}
	}
	 ...
	 * @return 指定项所在的页
	 */
	public void setItem(int itemOffset) {
		setPage((itemOffset / pageSize) + 1);
	}
	...
	public String toString() {
		...
		sb.append("    Showing items ").append(getBeginIndex()).append(" to ").append(getEndIndex()).append(" (total ").append(getItems())
				.append(" items), ");
